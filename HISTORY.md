...
# when using torshdev/meteor-launchpad:devbuild
...
- Add Docker build versioning to CI build.  Every tag in the form `vX.X.X` will now create and push a new build on CircleCI and update the `:latest` tag. You can now use `torshdev/meteor-launchpad:latest` to always use the latest, or you can pin a specific version with `torshdev/meteor-launchpad:v1.0.0`.  See all available builds here: https://hub.docker.com/r/torshdev/meteor-launchpad/tags/
